...
class ReferIt3DNet_transformer(nn.Module):
    ...
    def forward(self, batch: dict, epoch=None):
        # batch['class_labels']: GT class of each obj 48*88
        # batch['target_class']：GT class of target obj 48
        # batch['target_pos']: GT id 48
        if 'scene0255_00' in batch['scan_id']:
            print("here")
        ...
        obj_points2, boxs = self.aug_input(batch['objects2'], batch['box_info'])
        ## obj_encoding
        objects_features = get_siamese_features(self.object_encoder, obj_points, aggregator=torch.stack)
        objects_features2 = get_siamese_features(self.object_encoder, obj_points2, aggregator=torch.stack)


        # read pc1 and Ft1
        # with open("obj_points.pkl", 'rb') as obj_pointsfile:
        #     pc1 = pickle.load(obj_pointsfile)
        # with open("objects_features.pkl", 'rb') as objects_featuresfile:
        #     Ft1 = pickle.load(objects_featuresfile)
        # with open("cos_sim_ft13.pkl", 'rb') as cos_sim_ft13file:
        #     cos_sim_ft13 = pickle.load(cos_sim_ft13file)

        # with open("cls_id2pcs.pkl", 'rb') as cls_id2pcfile:
        #     cls_id2pc = pickle.load(cls_id2pcfile)
        
        # ft33=self.object_encoder(cls_id2pc['62'].to(self.device))
        # qwe=torch.cosine_similarity(ft33[0],self.cls_id2feat[62],dim=-1) # f(pc3)~g(pc3)should be 1
        # qwer=torch.cosine_similarity(objects_features[0][0],ft33[0],dim=-1) # f(pc3)~f(pc2)is 1
        #liu feaat bank
        ...
        # my_objects_features=torch.zeros(class_labels.shape+self.feat_dim).to(self.device)
        
        
        # for key, value in self.cls_id2feat.items():
        #     mask = class_labels == key
        #     my_objects_features += mask.unsqueeze(-1) * value


        # write pc1 and Ft1
        # with open("obj_points.pkl", 'wb') as obj_pointsfile:
        #     pickle.dump(obj_points, obj_pointsfile)        
        # with open("objects_features.pkl", 'wb') as objects_featuresfile:
        #     pickle.dump(objects_features, objects_featuresfile)
        # cos_sim_ft13=torch.cosine_similarity(objects_features[0],my_objects_features[0],dim=-1)
        # with open("cos_sim_ft13.pkl", 'wb') as cos_sim_ft13file:
        #     pickle.dump(cos_sim_ft13, cos_sim_ft13file)
        
        # print(torch.cosine_similarity(objects_features,my_objects_features,dim=-1))
